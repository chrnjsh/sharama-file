...
public class Tc_004lastNameTextBox {
	...
	public void launchBrowser(@Optional("chrome") String bname) {
		String[][] data=ReadData.readDataExcel("testData",24);
		...
		Assert.assertEquals(rp.getLastNametextBox().isDisplayed(),true);
		Reporter.log("click action on text box",true);
		String firstName  = data[0][0];
		rp.getLastNametextBox().sendKeys(firstName);
		Assert.assertEquals(rp.getLastNametextBox().getAttribute("value"),firstName,"data is not provided properly");
